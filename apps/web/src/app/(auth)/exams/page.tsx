...
import { Calendar, Plus, BarChart2, GraduationCap, FileText, ChevronRight, Loader2 } from 'lucide-react'
import type { ApiExam, ApiClass } from '@shared/types/api'
import { getAuth } from 'firebase/auth'

const ALLOWED_ROLES = ['admin', 'high_rank', 'academic', 'exam_officer', 'lower_rank', 'student'] as const
  ...
  return (
    <RoleGuard allowed={[...ALLOWED_ROLES]}>
      <div className="min-h-screen bg-page">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-6">
          ...
          {tab === 'exams' && (
            <div className="space-y-5">
                  ...
                  <>
                    ...
                    {selectedClassId && (
                      <button onClick={handleCompute} disabled={computing}
                        ...
                        {computing ? <Loader2 className="w-4 h-4 animate-spin" /> : <BarChart2 className="w-4 h-4" />}
                        Compute Results
                      </button>
                    )}
                  </>
              ...
              {examsLoading && <div className="text-center py-16 text-muted text-sm animate-pulse">Loading exams…</div>}
                    ...
                    <thead>
                      <tr className="bg-page border-b border-base">
                        {['Title','Subject','Date','Status','Actions'].map((h) => (
                          <th key={h} className="px-5 py-3 text-left text-xs font-heading font-semibold text-muted uppercase tracking-wide">{h}</th>
                        ))}
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-base">
                      {exams.map((exam) => (
                        <tr key={exam.id} className="hover:bg-page">
                          ...
                          <td className="px-5 py-3 text-muted">{new Date(exam.date).toLocaleDateString('en-MW')}</td>
                          <td className="px-5 py-3">
                            <span className={`text-xs font-semibold px-2 py-0.5 rounded-full ${
                              exam.status === 'RESULTS_RELEASED' ? 'bg-green-100 text-green-700'
                              : exam.status === 'RESULTS_APPROVED' ? 'bg-brand-teal/15 text-brand-teal'
                              : exam.status === 'MARKS_FINAL' ? 'bg-blue-100 text-blue-700'
                              : exam.status.includes('MARKS') ? 'bg-amber-100 text-amber-700'
                          ...
                          <td className="px-5 py-3">
                            <div className="flex items-center gap-2">
                              {canEnterMarks && ['SCHEDULED','IN_PROGRESS','MARKS_PENDING','MARKS_DRAFT'].includes(exam.status) && (
                                <button onClick={() => setMarksExamId(exam.id)}
                              ...
                              {canManage && exam.status === 'MARKS_FINAL' && (
                                <button onClick={() => approveResults.mutate(exam.id)}
                                  disabled={approveResults.isPending}
                                  className="text-xs text-brand-navy hover:underline">
                                  Approve
                                </button>
                              )}
                              {['admin','high_rank'].includes(role ?? '') && exam.status === 'RESULTS_APPROVED' && (
                                <button onClick={() => releaseResults.mutate(exam.id)}
          ...
          {/* ── ANALYTICS TAB ── */}
          {tab === 'analytics' && (
            <AnalyticsPanel academicYear={CURRENT_YEAR} selectedClassId={selectedClassId} term={term} />
          )}
        ...
        {marksExamId && (
          <MarksEntrySheet examId={marksExamId} classId={selectedClassId} onClose={() => setMarksExamId(null)} />
        )}
      </div>
    </RoleGuard>
  )
