...
import {
  ...
  Banknote, Eye, PiggyBank, Wallet2, Users,
} from 'lucide-react'
import { useMotionEnabled } from '@/store/motionStore'
import {
  LIST_CONTAINER_VARIANTS, LIST_ITEM_VARIANTS, reducedMotionVariants,
  reducedMotionTransition, DURATION, EASE,
} from '@/lib/motion'
...
const OUTCOME_CONFIG: Record<Outcome, { icon: React.ElementType; chip: string; label: string }> = {
  CREATED:  { icon: CheckCircle2, chip: 'bg-emerald-50 text-emerald-700 border-emerald-200 dark:bg-emerald-950/25 dark:text-emerald-400 dark:border-emerald-800/50', label: 'Created' },
  EXISTING: { icon: SkipForward,  chip: 'bg-blue-50 text-blue-600 border-blue-200 dark:bg-blue-950/25 dark:text-blue-400 dark:border-blue-800/50',                   label: 'Existing' },
  SKIPPED:  { icon: SkipForward,  chip: 'bg-page text-muted border-base',                                                                                              label: 'Skipped'  },
  ERROR:    { icon: XCircle,      chip: 'bg-rose-50 text-rose-700 border-rose-200 dark:bg-rose-950/25 dark:text-rose-400 dark:border-rose-800/50',                    label: 'Error'    },
}
...
function SummaryCards({ result }: { result: ApiBulkInvoiceResult }) {
  const cards = [
    { label: 'Invoices Created', value: result.created,  color: 'text-emerald-600', bg: 'bg-emerald-50 dark:bg-emerald-950/20' },
    { label: 'Already Existed',  value: result.existing, color: 'text-blue-600',    bg: 'bg-blue-50 dark:bg-blue-950/20' },
    { label: 'Skipped',          value: result.skipped,  color: 'text-muted',       bg: 'bg-page' },
    { label: 'Errors',           value: result.errors,   color: 'text-rose-600',    bg: 'bg-rose-50 dark:bg-rose-950/20' },
  ]
  return (
    <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
      ...
      <div className="col-span-2 sm:col-span-4 bg-brand-teal/8 border border-brand-teal/25 rounded-xl p-4 flex items-center gap-3">
        ...
        <div>
          <p className="font-heading font-bold text-brand-navy text-lg tabular">{formatMWK(result.totalRevenue)}</p>
  ...
  return (
    <div className="flex items-center gap-3 px-4 py-3 border-b border-base last:border-0 bg-surface">
      <div className="flex-1 min-w-0">
        ...
        <p className="text-xs text-muted">{row.registrationNo} &middot; {row.className}</p>
        {row.error && <p className="text-xs text-rose-600 mt-0.5">{row.error}</p>}
      </div>
      <div className="text-right shrink-0 space-y-1">
        <OutcomeBadge outcome={row.outcome} />
        {row.totalAmount !== undefined && <p className="text-xs text-muted tabular">{formatMWK(row.totalAmount)}</p>}
      </div>
    </div>
  )
}
...
}: { row: ApiBulkInvoiceStudentResult; checked: boolean; onToggle: () => void }) {
  const selectable = row.outcome === 'CREATED'
  return (
    <div className={`flex items-center gap-3 px-4 py-3 border-b border-base last:border-0 ${selectable ? 'bg-surface' : 'bg-page/60'}`}>
      <input
        type="checkbox"
      ...
      <div className="text-right shrink-0 space-y-1">
        <OutcomeBadge outcome={row.outcome} />
        {row.totalAmount !== undefined && <p className="text-xs text-muted tabular">Net: {formatMWK(row.totalAmount)}</p>}
        {!!row.priorArrears && <p className="text-[11px] text-rose-600 tabular">Arrears: +{formatMWK(row.priorArrears)}</p>}
      </div>
    </div>
  )
}
...
function ImpactProjection({ preview, selected }: { preview: ApiBulkInvoiceResult | null; selected: Set<string> }) {
  ...
  return (
    <div className="bg-brand-navy rounded-xl p-4 text-white space-y-2.5">
      ...
      <div className="border-t border-white/15 pt-2.5 flex items-center justify-between">
        ...
        <span className="font-heading font-bold text-lg tabular">{preview ? formatMWK(netReceivable) : formatMWK(0)}</span>
      </div>
    </div>
  )
}
...
export function BulkInvoiceGenerator() {
  ...
  async function runPreview() {
    ...
    try {
      ...
      setPreview(result)
      setSelected(new Set(result.students.filter((s) => s.outcome === 'CREATED').map((s) => s.studentId)))
    } catch (e) {
  ...
  const selectedClassName = classId === 'ALL' ? 'ALL classes' : `class ${classes.find((c) => c.id === classId)?.name ?? classId}`
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid lg:grid-cols-3 gap-4 items-start">
        ...
        <div className="bg-surface border border-base rounded-xl p-4 space-y-2.5">
          ...
          <RuleCheckbox
            ...
            checked={includeMandatory} onChange={setIncludeMandatory}
          />
          <RuleCheckbox
            ...
            checked={includeEnrolledOptional} onChange={setIncludeEnrolledOptional}
          />
          <RuleCheckbox
            ...
            checked={applyScholarships} onChange={setApplyScholarships}
          />
          <RuleCheckbox
            label="Consume Prior Advance Credit"
            hint="Offset new bills against any overpayment carried over from a prior term."
            checked={consumeAdvanceCredit} onChange={setConsumeAdvanceCredit}
          />
        </div>
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        ...
        <button
          type="button" onClick={runPreview} disabled={bulkGenerate.isPending || !academicYear}
        ...
        >
          {bulkGenerate.isPending && !finalResult ? <Loader2 className="w-4 h-4 animate-spin" /> : <Eye className="w-4 h-4" />}
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between flex-wrap gap-2">
            ...
            <div className="flex items-center gap-2">
              <button type="button" onClick={() => setSelected(new Set(eligibleRows.map((s) => s.studentId)))} className="text-xs font-medium text-brand-teal hover:underline">
                Select All Eligible
              </button>
              <button type="button" onClick={() => setSelected(new Set())} className="text-xs font-medium text-muted hover:underline">
                Deselect All
              </button>
            </div>
          </div>
          <div className="border border-base rounded-xl overflow-hidden">
            {[...eligibleRows, ...nonEligibleRows].map((row) => (
              <RosterRow key={row.studentId} row={row} checked={selected.has(row.studentId)} onToggle={() => toggleRow(row.studentId)} />
            ))}
          </div>
          <div className="flex justify-end">
            <button
              type="button" onClick={() => setConfirmOpen(true)} disabled={selected.size === 0 || bulkGenerate.isPending}
        ...
        <div className="space-y-5">
          ...
          <div className="flex gap-2 flex-wrap">
            {(['ALL', 'CREATED', 'EXISTING', 'SKIPPED', 'ERROR'] as const).map((o) => (
              <button
                key={o} type="button" onClick={() => setFilterOutcome(o)}
                className={`px-3 py-1.5 rounded-full text-xs font-heading font-semibold border min-h-9 ${
                  filterOutcome === o ? 'bg-brand-navy text-white border-brand-navy' : 'bg-surface border-base text-muted hover:border-brand-navy/30'
                }`}
              >
                {o === 'ALL' ? 'All' : o}
                {o !== 'ALL' && <span className="ml-1.5 opacity-75">({finalResult.students.filter((s) => s.outcome === o).length})</span>}
              </button>
            ))}
          </div>
          <motion.div
          ...
          >
            {filteredFinalStudents.length === 0 ? (
              <div className="text-center py-12 text-muted text-sm">No students match this filter.</div>
            ) : (
      ...
      {!preview && !finalResult && !bulkGenerate.isPending && (
        <div className="text-center py-16 text-muted text-sm border border-dashed border-base rounded-xl">
          Configure the options above and click Preview Roster to see exactly who would be billed before generating anything.
        </div>
      )}
