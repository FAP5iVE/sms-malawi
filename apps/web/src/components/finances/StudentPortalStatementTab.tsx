...
export function StudentPortalStatementTab({
  academicYear,
  term,
}: {
  academicYear: string
  ...
  const { data: feeSchedule = [], isLoading: scheduleLoading } = useFeeStructures(
    academicYear,
    undefined,
    term
  )
  ...
  const invoices = useMemo(() => balanceData?.invoices ?? [], [balanceData?.invoices])
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between gap-3 flex-wrap print:hidden">
        <div>
          <h2 className="font-heading font-semibold text-body">
            Student Fee Structure &amp; Financial Statement Portal
          </h2>
          <p className="text-xs text-muted mt-0.5">
            Your approved term fee schedule, verified payment receipts, and real-time account
            balance.
          </p>
        </div>
        <button
      ...
      <div className="bg-surface border border-base rounded-xl p-4">
        {isLoading ? (
          ...
          <p className="text-sm text-muted">
            We couldn&rsquo;t verify your student record. Please contact the school office.
          </p>
        ) : (
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              ...
              <p className="text-xs text-muted mt-0.5 flex items-center gap-3 flex-wrap">
                <span>
                  ID: <span className="font-mono">{me.registrationNo}</span>
                </span>
                <span className="inline-flex items-center gap-1">
                  <GraduationCap className="w-3 h-3" /> {me.className ?? 'Unassigned'}
                </span>
                <span className="inline-flex items-center gap-1">
                  <Phone className="w-3 h-3" /> Guardian: {me.guardianName}
                </span>
              </p>
            </div>
            <div className="text-right">
              ...
              <p
                className={`font-heading font-bold text-xl tabular ${currentBalance > 0 ? 'text-brand-coral' : 'text-emerald-600 dark:text-emerald-400'}`}
              >
      ...
      <div className="bg-surface border border-base rounded-xl overflow-hidden">
        ...
        {scheduleLoading ? (
          ...
          <p className="p-8 text-sm text-muted text-center">
            No approved fee schedule found for this term yet.
          </p>
        ) : (
              ...
              <thead>
                <tr className="border-b border-base bg-page">
                  <th className="text-left px-4 py-2.5 font-heading text-xs uppercase tracking-wide text-muted font-semibold">
                    Fee Line Particulars
                  </th>
                  <th className="text-left px-4 py-2.5 font-heading text-xs uppercase tracking-wide text-muted font-semibold hidden sm:table-cell">
                    Classification
                  </th>
                  <th className="text-left px-4 py-2.5 font-heading text-xs uppercase tracking-wide text-muted font-semibold hidden md:table-cell">
                    Billing Frequency
                  </th>
                ...
                {feeSchedule.map((f) => (
                  <tr key={f.id} className="border-b border-base last:border-0">
                    ...
                    <td className="px-4 py-3 text-xs text-muted hidden md:table-cell">
                      {FEE_SCHEDULE_LABELS[f.schedule as keyof typeof FEE_SCHEDULE_LABELS] ??
                        f.schedule}
                    </td>
            ...
            {payments.map((p) => (
              ...
              >
                <div className="text-sm">
                  <p className="font-medium text-body tabular">
                    {formatMWK(p.amount)}{' '}
                    <span className="font-normal text-muted">&middot; Term {p.term}</span>
                  </p>
