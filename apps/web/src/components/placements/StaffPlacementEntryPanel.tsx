...
export function StaffPlacementEntryPanel() {
  ...
  const visibleCandidates = useMemo(() => {
    ...
    return cohort.filter((c) => {
      ...
      if (!q) return true
      return `${c.firstName} ${c.lastName} ${c.candidateNo} ${c.registrationNo}`
        .toLowerCase()
        .includes(q)
    })
  ...
  const facultyGroups = useMemo(
    () => groupProgramsByFaculty(selectedUniversity),
    [selectedUniversity]
  )
  ...
  function handleSubmit() {
    ...
    if (!isCatalogue && !isFree) {
      setFormError(
        freeText
          ? 'Enter both a university name and a programme name.'
          : 'Choose a university, faculty and programme.'
      )
      return
    }
    if (!admissionYear.trim()) return setFormError('Enter the admission (intake) year.')
    if (!ncheBatchRef.trim())
      return setFormError('Cite the NCHE selection list / gazette reference.')
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-center justify-between gap-3 flex-wrap">
            <h3 className="font-heading font-semibold text-base">
              Staff Official Placement Entry (NCHE Harmonized Gazette)
            </h3>
            <AcademicYearSelect
              value={effectiveYear}
          ...
          <p className="text-xs text-muted mt-0.5">
            Cross-reference the published NCHE Selection List with our school cohort and register
        ...
        <div className="bg-surface border border-base rounded-xl p-4 space-y-3">
          ...
          {isLoading ? (
            ...
            <div className="text-sm text-brand-coral py-6 text-center px-4">
              <p className="font-medium">Could not load the cohort.</p>
            ...
            <div className="text-sm text-muted py-6 text-center px-4 space-y-1">
              <p>No certified-MSCE candidates found for {effectiveYear}.</p>
              <p className="text-xs">
                This means either no MSCE results have been marked Results Received or Certified yet
                for this year in the Exams module, or {effectiveYear} is not the right year, try a
                different academic year above.
              </p>
            </div>
          ) : visibleCandidates.length === 0 ? (
            <p className="text-sm text-muted py-6 text-center">No candidates match your search.</p>
          ) : (
                  ...
                  <button
                    ...
                    className={`w-full text-left rounded-xl border p-3 transition-colors ${
                      selected
                        ...
                        : 'border-base bg-page hover:border-brand-navy/40'
                    }`}
                  >
                    <div className="flex items-center justify-between gap-2">
                      <span className="font-medium text-sm flex items-center gap-1.5">
                        ...
                        <span
                          className={`text-[11px] font-bold ${c.sex === 'FEMALE' ? 'text-brand-coral' : 'text-brand-navy'}`}
                        >
                          {c.sex === 'FEMALE' ? 'F' : 'M'}
                        </span>
                      </span>
                      <span className="text-xs font-semibold shrink-0">
                        {c.aggregatePoints ?? '\u2014'} pts
                      </span>
                    </div>
                    <div className="flex items-center justify-between gap-2 mt-0.5">
                      <p className="text-xs text-muted">
                        {c.candidateNo} {'\u00b7'} ID: {c.registrationNo}
                      </p>
                      {c.existingStatus && (
                        <span className="text-[11px] font-medium text-muted">
                          {c.existingStatus === 'PENDING_APPROVAL'
                            ? 'Pending Approval'
                      ...
                      {!c.existingStatus && (
                        <span className="text-[11px] font-medium text-muted">Unplaced</span>
                      )}
                    </div>
                    <div className="flex flex-wrap gap-1 mt-1.5">
                      {Object.entries(c.subjectGrades)
                        .slice(0, 6)
                        .map(([subject, grade]) => (
                          <span
                            key={subject}
                            className="inline-flex items-center rounded bg-surface border border-base px-1.5 py-0.5 text-[11px] text-muted"
                          >
        ...
        <div className="bg-surface border border-base rounded-xl p-4 space-y-4">
          ...
          {!candidate ? (
            ...
            <>
              ...
              {freeText ? (
                <div className="grid gap-2 sm:grid-cols-2">
                  <input
                    type="text"
                    value={freeUniName}
                    onChange={(e) => setFreeUniName(e.target.value)}
                  ...
                  <input
                    type="text"
                    value={freeProgName}
                    onChange={(e) => setFreeProgName(e.target.value)}
                ...
                <>
                  <div>
                    <p className="text-xs font-medium text-muted mb-1.5">
                      Public University (1 of {catalogue.length} in Malawi)
                    </p>
                    <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                      {catalogue.map((u) => (
                        ...
                        >
                          ...
                          <p
                            className={`text-[11px] leading-snug ${universityId === u.id ? 'text-white/80' : 'text-muted'}`}
                          >
                            {u.name}
                          </p>
                        </button>
                      ...
                      >
                        ...
                        {facultyGroups.map((g) => (
                          <option key={g.faculty} value={g.faculty}>
                            {g.faculty} ({g.programs.length} programs)
                          </option>
                        ))}
                      </select>
                    </label>
                  ...
                  {faculty && (
                    <label className="text-sm block">
                      <span className="block text-xs text-muted mb-1">
                        Admitted Degree / Academic Program
                      </span>
                      <select
                        value={programmeId}
                      ...
                      >
                        ...
                        {facultyPrograms.map((p) => (
                          <option key={p.id} value={p.id}>
                            ...
                            {p.cutOffPoints ? ` (Cutoff: ~${p.cutOffPoints} pts)` : ''}
                          </option>
                        ))}
                      </select>
                    </label>
                    ...
                    <div className="bg-page border border-base rounded-xl p-3">
                      ...
                      <p className="text-xs text-muted mt-1.5">
                        {selectedProgramme.durationYears
                          ? `Duration: ${selectedProgramme.durationYears} Years \u00b7 `
                          : ''}
                        {selectedProgramme.cutOffPoints
                          ? `Typical Cutoff: ${selectedProgramme.cutOffPoints} pts \u00b7 `
                          : ''}
              ...
              {recordEntry.isError && (
                <p role="alert" className="text-sm text-brand-coral">
                  {(recordEntry.error as Error).message}
                </p>
              )}
              ...
              >
                ...
                {candidate.existingStatus === 'CONFIRMED'
                  ? 'Save Correction'
