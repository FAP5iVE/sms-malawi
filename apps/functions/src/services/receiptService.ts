...
import { uploadFile, getViewUrl, getDownloadUrl, STORAGE_BUCKETS } from '../lib/storage'
...
export async function generateReceipt(
  ...
  const fileId = await uploadFile(STORAGE_BUCKETS.PAYSLIPS, pdf, filename, 'application/pdf')
  return fileId
}
...
function buildReceiptHtml(data: {
  ...
  <div class="amount-row">
    ...
    <div class="amount-value">${formatMWK(data.amount)}</div>
  </div>
  <div class="footer">
...
function buildPayslipHtml(data: {
  ...
  <table>
    ...
    <tr><td>Basic Salary</td><td>${formatMWK(data.grossSalary)}</td></tr>
    <tr><td>PAYE Tax (deduction)</td><td>- ${formatMWK(data.paye)}</td></tr>
    <tr><td>Pension (5%)</td><td>- ${formatMWK(data.pension)}</td></tr>
    <tr><td>Loan Deduction</td><td>- ${formatMWK(data.loanDeduction)}</td></tr>
    <tr class="net"><td>NET PAY</td><td>${formatMWK(data.netSalary)}</td></tr>
  </table>
  </body></html>`
}
